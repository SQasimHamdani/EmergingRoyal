...
<head>
    ...
    <style>
        ...
        @media only screen and (max-width: 767px) { 
            .Emerging-responsive-form {
                text-align:  left !important;
            }
            
            .Emerging-responsive-form .form-group {
...
<nav class="navbar navbar-inverse">
    <div class="container-fluid">
        <div class="col-md-6 col-sm-12 col-xs-12">
            ...
            <div class="progress" id="progress-bar-Emerging">
                <div id="progress-bar-fill" class="progress-bar progress-bar-striped" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100"><span  style="font-size: 15px; font-weight: 600;  color: #f00" class="count-text">{{$general->add_show}}</span><span style="font-size: 15px; font-weight: 600; color: #f00">'s</span></div>
            </div>
        </div>
        <div class="col-md-6 col-sm-12 col-xs-12 text-right Emerging-responsive-form">

            <form class="form-inline">
 
                    <span id="Emerging_roy">
                        <div class="form-group">
                            <input  name="num1" id="cap_number_1"  value="{{rand(0,9)}}" type="text" readonly>
                        </div>
                        <div class="form-group"   >
                            <label for="exampleInputEmail2" style="color: #fff;"> + </label>
                            <input  name="num2" id="cap_number_2"  value="{{rand(0,9)}}" type="text"   readonly>
                        </div>
                        <div class="form-group"  >
                            <label for="exampleInputEmail2" style="color: #fff;">=</label>
                            <input name="result" type="number" id="cap_result"  required>
                        </div>
                        <div  class="form-group"  >
                            <div class="button-divgf">
                                <button type="button" class="btn btn-success navbar-btn cap_submit_btn" id="cap_submit_btn">Submit</button>
                            </div>
                        </div>
                    </span>
                <div  class="form-group">
                    <div class="button-div">
                        <button type="button" disabled class="btn btn-danger navbar-btn">Loading Add Please Wait {{$general->add_show}}s....</button>
                    </div>
                </div>
            </form>
...
<script>
    $(document).ready(function(){
        ...
        $('#Emerging_roy').hide();
        $('.progress').children().attr('id','progress-bar-Emerging');
        $('.progress-bar').children().attr('id','progress-bar-fill');
        ...
        var Main_Earn_button =  setTimeout(function(){
            $('.button-div').children('button').removeClass('btn-danger').addClass('btn-success').text('Enter Captcha to Complete');
            $('#Emerging_roy').show();
        ...
        $(document).on('click','#cap_submit_btn',function(){
            ...
            if (capResult == sum ){
               ...
               if (clickPer == 0) {
                   ...
                   $('.button-div').children('button').removeClass('btn-danger').addClass('btn-warning').text('Earned Money');
                   clickPer++;
               }
        ...
        // vis(function(){
        //     $('#progress-bar-Emerging').hide();
        //     $('#err').html("YOU SWITCHED THE TAB. RELOAD THE TAB AND WAIT");
        //     $('#Emerging_roy').hide();
        //     clearTimeout(Main_Earn_button);
    ...
    // // Preventing user to open the Developers Code
